...
```bash
VERSION=0.5.10; docker run -it --rm -v "$PWD:/photos" per2jensen/scrubexif:$VERSION "file1.jpg" "file2.jpeg"
```

#### Scrub all JPEGs in current directory

```bash
VERSION=0.5.10; docker run -it --rm -v "$PWD:/photos" per2jensen/scrubexif:$VERSION
```

#### Recursively scrub nested folders

```bash
VERSION=0.5.10; docker run -it --rm -v "$PWD:/photos" per2jensen/scrubexif:$VERSION --recursive
```
...
```bash
VERSION=0.5.10; docker run -it --rm \
...
```bash
VERSION=0.5.10; docker run -it --rm -v "$PWD:/photos" per2jensen/scrubexif:$VERSION --dry-run
```

Mix recursion and dry-run:

```bash
VERSION=0.5.10; docker run -it --rm -v "$PWD:/photos" per2jensen/scrubexif:$VERSION --recursive --dry-run
```
...
```bash
VERSION=0.5.10; docker pull per2jensen/scrubexif:$VERSION
```
...
```bash
VERSION=0.5.10; docker run --rm per2jensen/scrubexif:$VERSION --version
VERSION=0.5.10; docker run --rm per2jensen/scrubexif:$VERSION --help
```
...
```bash
VERSION=0.5.10; docker run --rm -v "$PWD:/photos" --entrypoint exiftool  per2jensen/scrubexif:$VERSION  "/photos/image.jpg"
```
...
```bash
VERSION=0.5.10; docker inspect per2jensen/scrubexif:$VERSION | jq '.[0].Config.Labels'
```

You can also check the digest and ID:

```bash
VERSION=0.5.10; docker image inspect per2jensen/scrubexif:$VERSION --format '{{.RepoDigests}}'
```
