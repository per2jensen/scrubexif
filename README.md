...
`$PWD/output/`:
    ...
    docker run --rm -v "$PWD:/photos" per2jensen/scrubexif:0.7.111
...
Same idea, but with container hardening and in-place overwrite (destructive):
    ...
    docker run -it --rm \
      ...
      --tmpfs /tmp \
      -v "$PWD:/photos" \
      per2jensen/scrubexif:0.7.111 --clean-inline
...
Use auto mode with explicit input/output/processed directories:
    ...
    docker run -it --rm \
      ...
      per2jensen/scrubexif:0.7.111 --from-input
...
`/etc/systemd/system/scrubexif.service`:
    ...
    ExecStart=/usr/bin/docker run --rm \
      ...
      per2jensen/scrubexif:0.7.111 --from-input --stable-seconds 10
