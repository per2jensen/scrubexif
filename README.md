...
Same idea, but with container hardening and in-line (destructive) overwrite:
    ...
    docker run -it --rm \
      ...
      --tmpfs /tmp \
      -v "$PWD:/photos" \
      per2jensen/scrubexif:0.7.18 --clean-inline
...
Use auto mode with explicit input/output/processed directories:
    ...
    docker run -it --rm \
      ...
      per2jensen/scrubexif:0.7.18 --from-input
...
```bash
cosign verify per2jensen/scrubexif:0.7.18 \
...
`/etc/systemd/system/scrubexif.service`:
    ...
    ExecStart=/usr/bin/docker run --rm \
      ...
      per2jensen/scrubexif:0.7.18 --from-input --stable-seconds 10
