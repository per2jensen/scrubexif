...
```bash
VERSION=0.7.20; docker run -it --rm \
  --read-only --security-opt no-new-privileges \
  --tmpfs /tmp \
  -v "$PWD:/photos" \
...
```bash
VERSION=0.7.20; docker pull per2jensen/scrubexif:$VERSION
```
...
```bash
VERSION=0.7.20; docker run --rm --read-only --security-opt no-new-privileges \
  --tmpfs /tmp \
...
```bash
VERSION=0.7.20; docker inspect per2jensen/scrubexif:$VERSION | jq '.[0].Config.Labels'
```

You can also check the digest and ID:

```bash
VERSION=0.7.20; docker image inspect per2jensen/scrubexif:$VERSION --format '{{.RepoDigests}}'
```
