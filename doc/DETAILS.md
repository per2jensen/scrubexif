...
```bash
VERSION=0.7.2; docker run -it --rm -v "$PWD:/photos" per2jensen/scrubexif:$VERSION "file1.jpg" "file2.jpeg"
```

#### Scrub all JPEGs in current directory

```bash
VERSION=0.7.2; docker run -it --rm -v "$PWD:/photos" per2jensen/scrubexif:$VERSION
```
...
```bash
VERSION=0.7.2; docker run -it --rm \
...
```bash
VERSION=0.7.2; docker run -it --rm -v "$PWD:/photos" per2jensen/scrubexif:$VERSION --dry-run
```

Mix recursion and dry-run:

```bash
VERSION=0.7.2; docker run -it --rm -v "$PWD:/photos" per2jensen/scrubexif:$VERSION --recursive --dry-run
```
...
```bash
VERSION=0.7.2; docker pull per2jensen/scrubexif:$VERSION
```
...
```bash
VERSION=0.7.2; docker run --rm per2jensen/scrubexif:$VERSION --version
...
```bash
VERSION=0.7.2; docker inspect per2jensen/scrubexif:$VERSION | jq '.[0].Config.Labels'
```
